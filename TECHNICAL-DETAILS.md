...
| `config`   | **Configuration.** Stores mod options (resolutions, caps) via JSON and persists them to disk.                                                                   | (Minecraft only)                                                   |
...
| `gui`      | Everything drawn on screen: the window base, the overlay coordinator, the image/video/audio windows, their registries, the hover preview, playlists and config. | `source`, `image`, `video`, `audio`, `media`, `playlist`, `config` |
...
| *(root)*   | The `@Mod` entry points (main mod + API). Fires `MediaSourceRegistrationEvent` during client setup.                                                             | `tools`, `api`, `gui`                                              |
...
| `MediaPlayerAPI.java`                                                 | The public façade. Static methods for source registration, playback control, volume, media queries, and playlist access. Delegates to internal classes. Exposes unique player IDs for fine-grained control of specific windows.                                                                                                                                                                                               |
...
| **`gui/`**                                                            |                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `MediaWindow.java`                                                    | Shared base for the on-screen windows. Owns the box geometry, the corner buttons (open-in-browser link, hide, close), the bottom-right resize grip, the move/resize/zoom gestures, and the global **z-order**. Each window is assigned a stable **ID** for API control. Declares the subclass contract, including a polymorphic `close()` and an `anchorGroup()` so the overlay never needs to know a window's concrete type. |
