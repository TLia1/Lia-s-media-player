...
A **client-side** mod for **NeoForge and Fabric** on Minecraft **1.21.1 through 26.3** (26.3 on Fabric only; primary target
...
| `storage`  | **One file on disk.** `JsonFileStore` reads and writes a file under `<gamedir>/liasmediaplayer/` as text, through a temp file and an atomic move, and never throws. The four stores share it and keep their own serialization. | *(root)*                                                           |
...
window swaps tracks. The same layer names their common transport: `media.MediaPlayback`
is the set of calls `VideoPlayer` and `AudioPlayer` had grown identically (play/pause,
the clock, seek, volume), and it is what lets `gui.QueuedMediaWindow` drive either
engine without either package knowing the other exists.

Inside `gui`, the video player and the audio bar are two subclasses of one pair of
classes rather than two parallel implementations: `QueuedMediaWindow<P>` owns the queue,
the docked panel, the transport and the seek/volume dragging, and
`PlayerWindowManager<W>` owns the window list, the front-most lookup and every
ID-addressed API method. Only what is genuinely per-engine — the picture, the control-bar
layout, the cap in the config — is left in the subclasses.

The mod reaches its own state through `MediaPlayerContext.getOrNull()` (anything on an
event bus, a tick or a render pass) or `MediaPlayerContext.get()` (anything that cannot
run before startup finished). Those two are the only places that cast the published API
instance back to the implementation.
...
| **`storage/`**                                                                              |                                                                                                                                                                                                                                                                                                                                                            |
| `JsonFileStore.java`                                                                          | One file under `<gamedir>/liasmediaplayer/`, read and written as text: `read()` gives the contents or `null`, `write(text)` goes to a sibling `.tmp` and an `ATOMIC_MOVE` over the target. The file handling all four stores used to hold a copy of. Never throws — a broken read or write is a log line and the caller keeps its defaults, because a mod that will not start until a preferences file is deleted is a worse failure than a forgotten window position. |
| **`media/`**                                                                                |                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `MediaPlayback.java`                                                                          | The transport `VideoPlayer` and `AudioPlayer` had both grown, named: url, start/dispose, play-pause, the clock, seek (absolute and by fraction), and the shared volume. It lives here for the reason everything shared does — a dependency from `video` to `audio` is what the package layout exists to prevent — and it is what lets one window class drive either engine. |
...
| **`gui/`**                                                                                  |                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `MediaWindow.java`                                                                          | Shared base for the on-screen windows, and the **coordinator** of the four classes below: it declares the subclass contract (source size, `drawContent`, `close()`, `anchorGroup()`, the transport, the persistence hooks), runs one layout pass per frame, publishes the box/content rects the subclass draws into, routes mouse events, and holds the window's stable ID and its place in the global z-order. The work itself belongs to `WindowPlacement`, `WindowGestures`, `WindowChrome`/`WindowButtons` and `WindowLinkActions`. |
| `WindowPlacement.java`                                                                      | Where a window is and how big, and the arithmetic that decides both: the scale (auto-fit, drag, or wheel zoom), the aspect ratio, the caps that keep the whole box — chrome included — on screen, theatre mode, the configured default corner, and the restore of a saved position/width. Pure Java; `WindowPlacementTest` covers it. |
| `WindowGestures.java`                                                                       | The mouse state a window keeps between events: which drag is running and where it was grabbed, whether two clicks paired into a double-click, where the cursor is and how long it has been still (theatre mode's idle timeout), and the click flash. Pure Java; `WindowGesturesTest` covers it. |
| `WindowChrome.java`                                                                         | Everything a window draws that is not its picture — title strip, corner buttons, resize grip, click flash — plus the vocabulary both player windows share for their control bars (the transport tooltips, the skip buttons, the colour of a toggle). All static and all handed its coordinates. |
| `WindowButtons.java`                                                                        | Where the corner-button row sits: heart, copy, browser link, optional hide, close, packed right-to-left, with a hit test per button and the width the row needs — which is part of a window's minimum size. `WindowButtonsTest` covers it. |
| `WindowLinkActions.java`                                                                    | What the corner buttons do with the link: keep it (history/favourites), open it in the system browser, copy it — plainly or with the current playback position written in, behind `Shift`. All three go through `Urls.isHttp` first. |
...
| `QueuedMediaWindow.java`                                                                      | The half of a player window that has nothing to do with pictures or sound: the `PlayQueue`, the docked `QueuePanel`, `advance`/`previous`/`jumpTo`, the player swap (dispose, build, start, announce), the keyboard transport, the seek/volume dragging, and the state a window restores. Generic over its player (`MediaPlayback`), so `VideoWindow` and `AudioWindow` keep theirs at its own type. A queue bug is fixed here once. |
| `VideoWindow.java`                                                                          | The on-screen player UI (extends `QueuedMediaWindow`): the video image and a control bar (play/pause, next, queue, loop, shuffle, speaker/volume pop-up, seek bar + time). The queue itself, and the reorderable panel docked beside it, come from the base class; shares layout math with `AudioWindow` via `MediaControls` and `Glyphs`.                                                                                                         |
| `PlayerWindowManager.java`                                                                    | The shared registry behind both player managers: the window list, `frontMost()` by z-order, queue-into-front-most vs. `open`, `playAll`, `hiddenCount`/`revealAll`, eviction past the cap, and every ID-addressed method the public API routes to. Subclasses supply two things — which window to build, and which config option caps them. |
| `VideoPlayerManager.java`                                                                   | Registry of active video windows — a `PlayerWindowManager<VideoWindow>` that builds `VideoWindow`s and reads `MAX_VIDEO_WINDOWS` (4) for its cap. Default behaviour is to **queue** a link into the front-most player; an independent window is only created on demand (shift-click) or when none exists.                                                                                                                                                                    |
| `AudioWindow.java`                                                                          | The compact audio **bar** (extends `QueuedMediaWindow`): a music note + the track name, and a control row (play/pause, previous, next, loop, shuffle, speaker, seek + time). No picture; backed by an `AudioPlayer`. It is the only window that wires "previous" to a control, though the queue traversal behind it is shared.                                                                                                                                                                 |
| `AudioPlayerManager.java`                                                                   | Registry of active audio bars — a `PlayerWindowManager<AudioWindow>` capped by `MAX_AUDIO_WINDOWS` (4). Same queue-into-front-most default as video and the same `playAll(urls, shuffle[, repeat])`; it adds only the two "previous" entry points the keybind and the API need.                                                                                                                                                                                 |
...
| **`image/`**                                                                                |                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `ImagePreviewCache.java`                                                                    | Bounded, lazy cache of downloaded previews keyed by URL. Downloads/decodes on a background IO pool, uploads textures on the main thread; the entry cap and the memory budget are both `MediaCache`'s (both are live config values). Resolves Tenor pages (via `TenorSource.isTenorPage` + `TenorResolver`) before downloading. One instance, owned by `MediaPlayerContext`.                                                                                    |
...
| `PlaylistStore.java`                                                                        | Loads/saves the playlists to `<gamedir>/liasmediaplayer/playlists.json` (Gson) via `storage/JsonFileStore`, lazily on first access and after every change, so a crash mid-write cannot corrupt the file.                                                                                                                                                                                                                                          |
| **`input/`**                                                                                |                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `BinaryDownloader.java`                                                                     | Downloads and installs tools when `BinaryLocator` finds nothing. Handles the two download shapes: a single executable (yt-dlp) and a per-platform archive (ffmpeg bundle). Unpacks `.zip` (JDK) and `.tar.xz` (system `tar`) with zip-slip protection. Uses a shared `HttpClient` and temp-file-then-atomic-move for safe writes. Pins ffmpeg to a release branch (`FFMPEG_RELEASE`) with `master` as a logged fallback, and refuses a payload whose magic bytes are not an executable for this platform. Carries the mod's [trust model](#external-tools-toolsmediabinaries-binarylocator-binarydownloader) in its class javadoc. |
| **`platform/`**                                                                             |                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
the chat, in whatever language they play in. (The bracketed forms quoted throughout this
document are the `en_us` values.) An addon's own source is expected to do the same with
keys from its own language files.
...
**Trust model.** This mod downloads executables and then runs them, so it owes an
explicit account of what that trust rests on. It rests on **TLS to three named
publishers** and on nothing else: `github.com/yt-dlp/yt-dlp` (the upstream project's own
release assets), `github.com/BtbN/FFmpeg-Builds` (the build most ffmpeg documentation
points Windows and Linux users at), and `evermeet.cx` (the equivalent for macOS).

There is deliberately **no checksum check**. None of the three publishes a checksum any
more trustworthy than the asset beside it — same host, same TLS, same release — so
verifying one would describe a guarantee the mod does not have. A hash baked into
`BinaryDownloader` instead would pin the bytes, but only until the next build replaces
them, which for yt-dlp is the point of the tool.

What *is* checked is what can be: a 200 response, a body large enough to be a real
binary rather than an error page, the **magic bytes** of an executable for the platform
we are on (`hasExecutableMagic` — `MZ` / `\x7fELF` / Mach-O, unit-tested), and archives
that cannot write outside their extraction directory (`safeResolve`). The magic-byte
check is what catches the plausible non-attacker failure: a moved asset, a captive-portal
page served with a 200, an archive whose layout changed and left the unpacker pointing at
a README — all of which otherwise end in `chmod +x` on a text file and a confusing
`Exec format error` at the first playback.

**Versions are pinned as far as each tool allows.** `BinaryDownloader.FFMPEG_RELEASE`
names an ffmpeg **release branch** (`n9.0`) rather than following `master`'s daily
autobuilds, so the decoder under the player only moves when that constant does; BtbN
publishes both families under the same stable `latest` release tag, so the URL keeps
working. `FFMPEG_FALLBACK_RELEASE` (`master`) is tried only if the pinned assets are
retired, and logs when it is. **yt-dlp deliberately stays on `latest`**: it breaks
whenever YouTube changes its player, so being current *is* its function — see *Keeping
yt-dlp current* below.
...
`PlaylistStore` and `WindowStateStore`: lazy load, and a write through
`storage/JsonFileStore` — a temp file plus an atomic move — that is never thrown from. Entries are most-recent-first; playing something already in the list moves
...
Every target compiles with `-Xlint:deprecation,unchecked,rawtypes,fallthrough`, and the
tree is currently clean of all four. **Checkstyle**, **JaCoCo** and `-Werror` run on the
single target named by `quality.target` in `gradle.properties` (`1.21.1-neoforge`, the VCS
target):

```
./gradlew :1.21.1-neoforge:checkstyleMain :1.21.1-neoforge:checkstyleTest
./gradlew :1.21.1-neoforge:jacocoTestReport
```

One target rather than fourteen because all fourteen compile the same rewritten `src/`, so
the rest would be the same report over again — and because `-Werror` on all of them would
turn a Minecraft deprecation on a version nobody has ported yet into a matrix-wide build
failure instead of a warning to work through.

The ruleset in `config/checkstyle/checkstyle.xml` is deliberately short: inline
fully-qualified names of the mod's own classes (the one that keeps coming back), star
imports, redundant imports, line length, `FallThrough`, `EmptyStatement`, `EqualsHashCode`.
It does **not** include `UnusedImports`, and that is a Stonecutter consequence rather than
an oversight: the branch of a `//?` guard the active target does not select sits inside a
`/* */`, so an import only that branch needs reads as unused here — `ChatHitTest`,
`ModKeybinds` and `Tooltips` each have one. Acting on that finding trades a harmless unused
line for a build failure on a version the author was not looking at.

Coverage is published, without a threshold. Most of this codebase is rendering and process
plumbing no unit test can reach, so a bar would be either meaningless or an incentive to
test the wrong things; the number is there so the direction is visible. It currently sits
around **23 % of lines** (29.8 % of methods), across 39 test classes.

CI mirrors all of this: a *Lint & coverage* job runs Checkstyle and JaCoCo on that target
and uploads both reports, the per-target matrix runs `test` as its own step before `build`
(so a failing test is named in the job list rather than buried in a build failure), and the
matrix-parsing job also asserts that the committed `stonecutter active` target matches
`quality.target` — a commit made from a switched tree rewrites every guard in `src/`.
...
- **Shared queue.** `gui/PlayQueue` is the one queue model for both player windows, and
  `gui/QueuedMediaWindow` is the one class that drives it: appending, advancing, going
  back, swapping the player, and restoring the panel/loop/shuffle state on load. It also
  owns the history, the `RepeatMode` and the shuffle flag — put any new "what plays next"
  rule in one of those two, never in `VideoWindow` or `AudioWindow`, so both players get
  it. The same goes for the registries: `gui/PlayerWindowManager` holds everything the
  video and audio managers do, and each subclass supplies only its window factory and its
  cap.
- **One persistence path.** Every store writes through `storage/JsonFileStore`. Do not
  hand-roll another `createDirectories` / `.tmp` / `ATOMIC_MOVE` / `catch (IOException |
  RuntimeException)` block — that repetition across four classes is exactly what it
  replaced.
- **Reaching the context.** `MediaPlayerContext.getOrNull()` for anything that can run
  before startup finished (event-bus listeners, ticks, rendering) and
  `MediaPlayerContext.get()` for anything that cannot. They are the only two places that
  cast the published API instance back to the implementation; a call site should never
  write the cast itself.
