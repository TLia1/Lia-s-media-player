...
A **client-side** NeoForge mod for **Minecraft 1.21.1 through 26.2** that turns image,
GIF, video and audio links shared in the in-game chat into rich, watchable (and
listenable) media — without ever leaving the game.
...
One jar per Minecraft version, all built from the same source tree. Download the
one matching your game; they are not interchangeable.
...
1. Install the NeoForge build matching your Minecraft version — see
...
Java and Parchment versions. They are expanded into
`src/main/resources/META-INF/neoforge.mods.toml` at build time, and feed the
generated table in [Supported versions](#supported-versions).
...
`GuiGraphicsExtractor` family on 26.1+. They live in `build.gradle.kts` and are
bidirectional, so switching the active version rewrites `src/` both ways.
